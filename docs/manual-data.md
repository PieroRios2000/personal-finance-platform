...
`gold.fct_investment_monthly`: a month shows a return only if it **closes at a `valorizacion` row**
(its last row) and follows the previous month; the first month of a fund needs an `aporte` or
`retiro`. It is safe
