...
Both sheets are loaded into bronze (`Ahorros` as statements, `Inversiones` as its
own table); then run the pipeline as usual (`dbt build`). The return of each fund and month is in
`gold.fct_investment_monthly`: a month's closing balance is its **last row**; if that row is a `valorizacion` the return
is marked `closing_basis = valuation`, otherwise `last_movement` (accepted for the months typed
before valuations existed; from now on add a `valorizacion` at every month end). A month shows a
return if it has capital and follows the previous month; the first month of a fund needs an
`aporte` or `retiro`. It is safe
