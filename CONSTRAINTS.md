...
| disabled-test | `tests/test_dbt_gold_integration.py` | T23's own gold star-schema proof (`fact_transactions` row count matches `silver.transactions`, every FK's `relationships` test runs and passes, `flow_type`'s mapping for all four `account_kind`/sign combinations, the cross-bank BCP/Scotiabank transfer scenario, the four-dimension "spend by bank by month" query) — identical pattern and identical reason to the already-approved `test_dbt_internal_transfers_integration.py` row above, including its own local `lake` fixture for the same F811 reason. Needs a live SeaweedFS with `.env` exported; T17's `ephemeral-integration` job runs it for real in CI. | Piero | 2026-12-16 |
...
| disabled-test | `tests/test_elementary_anomaly_integration.py` | T22's own proof that Elementary's `volume_anomalies` test fires on a synthetic row-count spike and not on a normal day (ADR 0022), same pattern as the already-approved `test_dbt_silver_integration.py`/`test_dbt_gold_integration.py` rows above, its own local `lake` fixture for the same F811 reason those rows already give. Needs a live SeaweedFS with `.env` exported; `ephemeral-integration` runs it for real in CI. | Piero | 2026-12-18 |
| disabled-test | `tests/test_dbt_postgres_integration.py` | T26's proof that dbt builds silver and gold into PostgreSQL through DuckDB's attach (tables land in Postgres and Elementary's do not, a second build changes nothing, the read-only BI role reads `gold` only and keeps doing so after dbt recreates the tables). Same pattern and reason as the approved `test_dbt_silver_integration.py` row: it needs a live SeaweedFS **and a live PostgreSQL** with `.env` exported (`make poc-up`, `make pg-up`), so a plain local `pytest` or CI's own `tests` job has to skip it rather than fail; `ephemeral-integration` runs it for real (its Postgres arrives with this task). | Piero | 2026-12-19 |
| disabled-test | `tests/test_dex_config.py` | T39's two `gomplate`-render tests skip when that binary isn't installed (`shutil.which`) -- it's the official Dex image's own bundled templating tool (not a project dependency, no CI job installs it), used only to prove `dex/config.yaml.tpl` renders to valid, expected YAML the same way the container does at startup. `test_the_template_never_hardcodes_a_user_or_a_client_secret` (static, unconditional) and Superset actually completing an OIDC login are the PR's real coverage; these two are a bonus check for whoever has `gomplate` locally. | Piero | 2026-12-21 |
