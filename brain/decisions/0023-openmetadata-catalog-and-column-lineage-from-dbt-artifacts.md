...
## Update 2026-09-20 (T30)

The DuckDB workaround this ADR describes (no DuckDB connector in OpenMetadata, hence a script registering the tables) is retired for silver and gold: dbt stores them in PostgreSQL ([ADR 0029](0029-dbt-stores-silver-and-gold-in-postgres.md)) and OpenMetadata reads them with its native Postgres connector (`metadata ingest` type `postgres`, schemas `silver` and `gold`, service `pfp_postgres`). The `ingestion` container joins the Docker network of PFP's Postgres to reach it. `openmetadata_sync.py` shrinks to what the connector cannot cover: it registers the service and the **bronze** tables (Delta on S3, typed from `lakehouse/bronze.py`), writes both workflow configs and the manifest copy with `delta_scan(...)` resolved, and `check` runs the column-lineage assertion. The column-level lineage from dbt's artifacts stays, and `fact_transactions.amount -> silver.transactions.amount -> bronze.transactions.amount` was verified on a live server (native connector: 14 tables, 0 errors; dbt workflow: 0 errors). Also fixed: `bronze_tables` did not know the `investment_entries` source. Elementary's tables live in a separate DuckDB file, so they are not catalogued.
