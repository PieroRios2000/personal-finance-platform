...
| Piece | What it does |
|---|---|
| [`dbt/profiles.yml`](../../dbt/profiles.yml) | The connection. Since T26/T27 (ADR 0029) the default target is `postgres`: DuckDB is the engine and silver and gold are stored in PostgreSQL through `attach` (`PFP_PG_*`), with Elementary on its own DuckDB file (`PFP_ELEMENTARY_DUCKDB_PATH`). `PFP_DBT_TARGET=local` selects the earlier on-disk DuckDB database (`PFP_DUCKDB_PATH`, default `dbt/pfp.duckdb`). Both targets: schema `silver`, the `httpfs` and `delta` extensions, and the `TYPE s3 / PROVIDER config` secret that lets DuckDB reach SeaweedFS (`ENDPOINT`, `URL_STYLE 'path'`, `USE_SSL false`). Every value comes from the same `.env` variables `lakehouse/storage.py` reads |
...
| [`dbt/models/silver/investment_entries.sql`](../../dbt/models/silver/investment_entries.sql) | Phase 6 (ADR 0028): the manual Excel's investment rows, typed, with their month; an empty table when the lake has no investments (`bronze_table_exists`). Not part of `silver.transactions` |
