...
| [`dbt/models/gold/dim_date.sql`](../../dbt/models/gold/dim_date.sql) | `gold.dim_date`: the shared calendar, one row per day from the first day of the first month to the last day of the last month any fact has data for (T34; before that only the dates with a movement), with date parts, `year_month` and `year_quarter` |
