...
| [`dbt/models/gold/dim_date.sql`](../../dbt/models/gold/dim_date.sql) | `gold.dim_date`: the shared calendar, one row per day from the first day of the first month to the last day of the last month any fact has data for (T34; before that only the dates with a movement), with date parts, `month_label` and `year_quarter` |
| [`dbt/models/gold/rpt_movements.sql`](../../dbt/models/gold/rpt_movements.sql), `rpt_balances.sql`, `rpt_investments.sql` | Reporting tables (T34): each fact joined to `dim_date`, all with the same `calendar_year`/`calendar_quarter`/`calendar_month` columns, so one BI filter applies to every chart. `rpt_balances` also has `month_recency` (1 = latest month). Tables, not views: dbt-duckdb cannot create a view in the attached Postgres |
