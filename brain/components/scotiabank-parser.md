...
- **Row dates**: `DD/MM/YY`, two columns — only the *first* is kept (the owner's choice; the second is
  ignored), the earlier one given a throwaway column so it can't bleed into it. Unlike
