...
def _storage_metadata(key: dg.AssetKey) -> dict[str, Any]:
    """Where a model is stored and how many rows it has. Empty for a model this
    graph does not know, and without a row count on the `local` (DuckDB file) target,
    which has no Postgres to ask."""
...
def dbt_models(context: dg.AssetExecutionContext, dbt: DbtCliResource) -> Iterator[Any]:
    ...
    for event in dbt.cli(_dbt_build_args(), context=context).stream():
        if isinstance(event, dg.Output):
            ...
            event = event.with_metadata(_storage_metadata(key))
        yield event
